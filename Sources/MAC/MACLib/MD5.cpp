...
#define FF(a, b, c, d, x, s, ac)     { (a) += F (b, c, d) + (x) + static_cast<uint32_t>(ac); (a) = ROTATE_LEFT (a, s); (a) += (b); }
#define GG(a, b, c, d, x, s, ac)     { (a) += G (b, c, d) + (x) + static_cast<uint32_t>(ac); (a) = ROTATE_LEFT (a, s); (a) += (b); }
#define HH(a, b, c, d, x, s, ac)     { (a) += H (b, c, d) + (x) + static_cast<uint32_t>(ac); (a) = ROTATE_LEFT (a, s); (a) += (b); }
#define II(a, b, c, d, x, s, ac)     { (a) += I (b, c, d) + (x) + static_cast<uint32_t>(ac); (a) = ROTATE_LEFT (a, s); (a) += (b); }
         ...
         int             repeat )
{
    ...
    if ( intn(in) & 3 ) {
        ...
        x = reinterpret_cast<const uint32_t*>(in);
    }
#endif
        ...
        int64          inputBytes )
{
    ...
    byteIndex = static_cast<int>((context -> count[0] >> 3) & 0x3F);

    /* Update number of bits: count += 8 * inputBytes */
    const uint32 s = static_cast<uint32>(inputBytes << 3);
    if ( (context -> count [0] += s) < s )
        context -> count [1]++;
    context -> count [1] += static_cast<uint32>(inputBytes >> (32 - 3));

    partLen = static_cast<unsigned int>(64 - byteIndex);

    /* Transform as many times as possible. */
    if ( inputBytes >= partLen ) {
    memcpy ( context -> buffer + byteIndex, input, partLen );
    MD5Transform ( context -> state, static_cast<const uint8_t *>(context -> buffer), 1 );
    len       = static_cast<int>(( inputBytes - partLen ) >> 6);
    MD5Transform ( context -> state, input + partLen, len );
    i         = static_cast<int>(partLen) + static_cast<int>(len << 6);
    byteIndex = 0;
    }
    else {
    i         = 0;
    ...
    memcpy ( (context -> buffer) + byteIndex, input + i, static_cast<size_t>(inputBytes - i) );
}
       ...
       MD5_CTX*  context )
{
    ...
    byteIndex        = int((context -> count[0] >> 3) & 0x3F);
    ...
    MD5Update ( context, reinterpret_cast<const uint8_t *>(bits), 8 );
...
bool CMD5Helper::GetResult(unsigned char cResult[16])
{
    memset(cResult, 0, 16);
