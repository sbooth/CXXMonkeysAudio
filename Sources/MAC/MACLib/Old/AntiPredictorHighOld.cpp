...
void CAntiPredictorHigh0000To3320::AntiPredict(int * pInputArray, int * pOutputArray, int NumberOfElements)
{
    ...
    {
        const int * pBack = &pOutputArray[q - 5];
        p = (5 * pBack[4]) - (10 * pBack[3]) + (12 * pBack[2]) - (7 * pBack[1]) + pBack[0];
    ...
    for (q = 8; q < NumberOfElements; q++)
    {
        const int * pBack = &pInputArray[q - 4];
        p = (4 * pBack[3]) - (6 * pBack[2]) + (4 * pBack[1]) - pBack[0];
        pw = (p * m) >> 12;

        const int * pOut = &pOutputArray[q];
        pInputArray[q] = *pOut + pw;
...
void CAntiPredictorHigh3320To3600::AntiPredict(int * pInputArray, int * pOutputArray, int NumberOfElements)
{
    ...
    AntiPredictorOffset.AntiPredictOffset(pInputArray, pOutputArray, NumberOfElements, 2, 12);
    AntiPredictorOffset.AntiPredictOffset(pOutputArray, pInputArray, NumberOfElements, 3, 12);

    AntiPredictorOffset.AntiPredictOffset(pInputArray, pOutputArray, NumberOfElements, 4, 12);
    AntiPredictorOffset.AntiPredictOffset(pOutputArray, pInputArray, NumberOfElements, 5, 12);

    AntiPredictorOffset.AntiPredictOffset(pInputArray, pOutputArray, NumberOfElements, 6, 12);
    AntiPredictorOffset.AntiPredictOffset(pOutputArray, pInputArray, NumberOfElements, 7, 12);
...
void CAntiPredictorHigh3600To3700::AntiPredict(int * pInputArray, int * pOutputArray, int NumberOfElements)
{
    ...
    memcpy(pOutputArray, pInputArray, static_cast<size_t>(13 * 4));
    ...
    for (q = 13; q < NumberOfElements; q++)
    {
        int * pOut = &pOutputArray[q];
        IP0 = *pOut + ((p4 * m4) >> 9) - ((p6 * m6) >> 10);
        (*pOut ^ p4) >= 0 ? m4++ : m4--;
        (*pOut ^ p6) >= 0 ? m6-- : m6++;
        p4 = (2 * IP0) - IP1;
        p6 = IP0;
...
void CAntiPredictorHigh3700To3800::AntiPredict(int * pInputArray, int * pOutputArray, int NumberOfElements)
{
    ...
    int bm[FIRST_ELEMENT]; APE_CLEAR(bm);
    ...
    for (;op < &pOutputArray[NumberOfElements]; op++, ip++)
    {
        ...
        *ip = Original - (((ip[-1] * bm[0]) + (ip[-2] * bm[1]) + (ip[-3] * bm[2]) + (ip[-4] * bm[3]) + (ip[-5] * bm[4]) + (ip[-6] * bm[5]) + (ip[-7] * bm[6]) + (ip[-8] * bm[7]) + (ip[-9] * bm[8]) + (ip[-10] * bm[9]) + (ip[-11] * bm[10]) + (ip[-12] * bm[11]) + (ip[-13] * bm[12]) + (ip[-14] * bm[13]) + (ip[-15] * bm[14]) + (ip[-16] * bm[15])) >> 8);
        ...
        {
            bm[0] -= ip[-1] > 0 ? 1 : -1;
            bm[1] += ((static_cast<uint32>(ip[-2]) >> 30) & 2) - 1;
            bm[2] -= ip[-3] > 0 ? 1 : -1;
            bm[3] += ((static_cast<uint32>(ip[-4]) >> 30) & 2) - 1;
            bm[4] -= ip[-5] > 0 ? 1 : -1;
            bm[5] += ((static_cast<uint32>(ip[-6]) >> 30) & 2) - 1;
            bm[6] -= ip[-7] > 0 ? 1 : -1;
            bm[7] += ((static_cast<uint32>(ip[-8]) >> 30) & 2) - 1;
            bm[8] -= ip[-9] > 0 ? 1 : -1;
            bm[9] += ((static_cast<uint32>(ip[-10]) >> 30) & 2) - 1;
            bm[10] -= ip[-11] > 0 ? 1 : -1;
            bm[11] += ((static_cast<uint32>(ip[-12]) >> 30) & 2) - 1;
            bm[12] -= ip[-13] > 0 ? 1 : -1;
            bm[13] += ((static_cast<uint32>(ip[-14]) >> 30) & 2) - 1;
            bm[14] -= ip[-15] > 0 ? 1 : -1;
            bm[15] += ((static_cast<uint32>(ip[-16]) >> 30) & 2) - 1;
        }
        else if (Original < 0)
        {
            bm[0] -= ip[-1] <= 0 ? 1 : -1;
